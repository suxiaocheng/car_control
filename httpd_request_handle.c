...
int
read_file(char *filename, char **pbuf) 
{
	int ret = -1;
	int fd;
	...
	if (filename != NULL){
		...
		if(ret != -1) {
			...
			*pbuf = (char *)malloc(file_size+1);
			if (*pbuf != NULL) {
				ret = read(fd, *pbuf, file_size);
				if (ret != file_size) {
					ERR("read %s filesize mismatch, read%d, except:%d\n", 
							filename, ret, file_size);
					free(*pbuf);
					*pbuf = NULL;
					ret = -1;
				} else {
...
add_session_cookie(struct Session *session, struct MHD_Response *response)
{
	...
	if (MHD_NO ==
	    MHD_add_response_header(response,
				    MHD_HTTP_HEADER_SET_COOKIE, cstr)) {
		  ...
		  struct Session *session, struct MHD_Connection *connection)
{
	...
	MHD_add_response_header(response,
				MHD_HTTP_HEADER_CONTENT_ENCODING, mime);
	     ...
	     struct Session *session, struct MHD_Connection *connection)
{
	...
	if (session->raw_html == NULL) {
		ret = read_file("index.html", &session->raw_html);
	} else {
		ret = strlen(session->raw_html);
	}
	if((ret == -1) || (ret == 0)) {
		reply = NOT_FOUND_ERROR;
		ret = strlen(reply);
	} else {
		time_t timep;
		struct tm *p;
		...
		time(&timep);
		p = localtime(&timep);
		snprintf(time_tag, sizeof(time_tag), "%d:%d:%d", p->tm_hour, p->tm_min, p->tm_sec);

		reply = malloc(ret + 512);
		snprintf(reply, ret+512, session->raw_html, time_tag);

		ret = strlen(reply);
	}

	/* return static form */
	response = MHD_create_response_from_buffer(ret,
						   (void *)reply,
		...
		struct Session *session, struct MHD_Connection *connection)
{
	...
	reply =
	    malloc(strlen(form) + strlen(session->value_1) +
		   strlen(session->value_2) + 1);
	snprintf(reply,
		 strlen(form) + strlen(session->value_1) +
		 strlen(session->value_2) + 1, form, session->value_1);
	/* return static form */
		...
		const char *upload_data, size_t * upload_data_size, void **ptr)
{
	...
	if ((0 == strcmp(method, MHD_HTTP_METHOD_GET)) ||
		...
		ret = pages[i].handler(pages[i].handler_cls,
				       pages[i].mime, session, connection);
		if (ret != MHD_YES)
	...
	ret = MHD_queue_response(connection,
				 MHD_HTTP_METHOD_NOT_ACCEPTABLE, response);
	MHD_destroy_response(response);
	return ret;
}
